...
print(phi.shape)
#phi_stack=phi.reshape(K*phi.shape[0])
#phi_mod=100*np.ones(3600)*phi_stack
#phi_test=(phi_stack*phi_mod).reshape(-1,K)
#print(phi_stack.shape)
# --- Plot first 4 basis functions (or fewer if less nodes) ---
a = np.arange(start=0,stop=9)
print(a)
phi_mod=phi@a
...
for i in range(num_plots):
    #Z = phi_mod[:, i].reshape(X.shape)
    Z = phi_mod.reshape(X.shape)
    ax = axes[i]
