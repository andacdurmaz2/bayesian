import numpy as np
from src.conditionals import beta_draw, b_draw, sigma_b_draw, sigma_e_draw, sigma_b_draw_MH




def run_one_gibbs(y, X, sigma_b, sigma_e, b, beta, priors,sigma_b_var_curr):
    """
    ...
    sigma_b_new,sigma_b_var = sigma_b_draw_MH(
        beta=beta_new,
        b=b_new,
        y=y, # (unused, but for signature)
        n=n,
        var=sigma_b_var_curr,
        a_rho=1,
        b_rho=1,
        a_sgm=1,
        b_sgm=1
    ...
    return sigma_b_new, sigma_e_new, b_new, beta_new, sigma_b_var
...
def run_mcmc_mh(y_list, X_list, priors, n_iter=2000, n_burn=1000):
    ...
    sigma_b_var_samples=np.zeros((n_samples,2))
    ...
    for i in range(n_iter):
        # Run one iteration
        sigma_b_curr, sigma_e_curr, b_curr, beta_curr,sigma_b_var_curr = run_one_gibbs(
            y_list, X_list,
            sigma_b_curr, sigma_e_curr, b_curr, beta_curr,
            priors,sigma_b_var_curr
        )
